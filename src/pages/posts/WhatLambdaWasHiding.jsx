  ...
  return (
    <div className="blog-post-container">
      ...
      <article className="article-content">
        ...
        <p>
          Some we caught before rollout. Some production caught for us.
        </p>
        ...
        <h3 id="case-1">1. <code>/tmp</code> collisions and a duplicate-delivery discovery</h3>
        <p>
          On Lambda, concurrent invocations run in separate execution environments, each with its own <code>/tmp</code>. Two deliveries in flight at once never share a filesystem, so writing intermediate files to <code>/tmp</code> is safe from collisions by construction. Sequential warm invocations could share <code>/tmp</code>, but that was never the collision that mattered. In the new service, all in-flight deliveries share one process and one <code>/tmp</code> – two deliveries with the same filename collide.
        </p>
        <p>
        ...
        <p>
          None of this is novel – reusing the source's delivery UUID for downstream state is a standard pattern (Kleppmann covers it in <a href="https://www.oreilly.com/library/view/designing-data-intensive-applications/9781098119058/" target="_blank" rel="noopener noreferrer"><i>Designing Data-Intensive Applications</i></a>), and the temp path is just one more place to apply it. This applies only to the Python consumers. The Go extraction service holds intermediate state in memory and doesn't touch <code>/tmp</code>.
        </p>
        <p>
        ...
        <p style={{ fontStyle: 'italic', color: 'var(--text-secondary)' }}>
          A careful reader might ask why the guardrail went to the producer rather than the consumers, where idempotency is usually enforced. The answer is that the two layers guard against different things. At the producer, the same delivery should not be extracted twice at the same time – that's waste. At the consumers, we deliberately did not want to reject duplicates in general. Replaying from an older offset is how we recover from bugs, and a consumer that refuses anything it's seen before can't be replayed. The <code>/tmp</code>-path collision only catches concurrent duplicates – the same delivery in flight twice at once – which is exactly the scope we wanted. Sequential redelivery stays possible, the database-level idempotency makes it safe.
        </p>
        ...
        <p>
          On Lambda, the runtime handled this for us. SNS invokes Lambda asynchronously, and Lambda <a href="https://docs.aws.amazon.com/lambda/latest/dg/invocation-async-error-handling.html" target="_blank" rel="noopener noreferrer">retries a failed async invocation</a> on its own. A network blip failed a call. Lambda ran the invocation again. A later attempt went through, and the application code never had to think about it.
        </p>
        <p>
        ...
        <p>
          The fix was the obvious one: wrap the HTTP call in retry-with-backoff, give up after a few attempts, let the queue handle anything that exceeds that. Because the pipeline is idempotent at insertion – as I noted in <a href="#case-1">Case 1</a> – aggressive in-process retry is safe.
        </p>
        <p>
        ...
        <p>
          The reason was simple. The original code opened a new <code>aiohttp.ClientSession</code> for every outbound call, and closed it right after – no reuse:
        </p>
    ...
    async with session.post(url, json=payload) as resp:
        ...
        <p>
          The fix was to create one <code>aiohttp.ClientSession</code> at service startup with a connection pool limit, and reuse it for every call. With a bounded pool, the connection count stayed below the upstream's per-source limit.
        </p>
        <pre><code className="language-python" dangerouslySetInnerHTML={{ __html: Prism.highlight(`# at startup, once
session = aiohttp.ClientSession(
    connector=aiohttp.TCPConnector(limit=MAX_CONNECTIONS)
        ...
        <p>
          The fix was <code>asyncio.to_thread()</code>. Sync DB clients and blocking calls were deferred to a thread pool, freeing the event loop. The tradeoff: thread pool exhaustion degrades gracefully (tasks queue) while event loop blocking fails silently. The thread pool still has a size, and that size is now a tunable. Set it too small and deliveries queue behind slow upserts. Set it too large and too much heavy work runs at once. Async code is only as async as its slowest sync call.
        </p>
        ...
        <p>
          For years the system ran on Lambda with these assumptions intact – some of them latent bugs, some responsibilities Lambda had been handling for us. The runtime never made any of it visible. It just absorbed it, and nobody noticed. That's the part that matters, more than the count. Any production system on a managed runtime is making the same bet, whether the team running it knows it or not.
        </p>
        ...
        <div className="table-container" style={{ overflowX: 'auto', margin: '2.5rem 0' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.95rem' }}>
            ...
            <tbody>
              ...
              <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                <td style={{ padding: '0.75rem 1rem' }}>Upstream connection refusals</td>
                <td style={{ padding: '0.75rem 1rem' }}>Connections spread over more egress IPs</td>
