  ...
  return (
    <div className="blog-post-container">
      ...
      <article className="article-content">
        ...
        <p>
          On Lambda, concurrent invocations run in separate execution environments, each with its own <code>/tmp</code>. Two deliveries in flight at once never share a filesystem, so writing intermediate files to <code>/tmp</code> is safe from collisions by construction. Sequential warm invocations could share <code>/tmp</code>, but they did not write concurrently, so they could not collide. In the new service, all in-flight deliveries share one process and one <code>/tmp</code> – two deliveries with the same filename collide.
        </p>
        <p>
        ...
        <p>
          I used the delivery UUID for the destination directory. An atomic <code>mkdir</code> let the first delivery claim the path, so an overlapping duplicate on the same instance failed loudly. Cleanup preserved sequential replay.
        </p>
        <p>
...
session = aiohttp.ClientSession(
    connector=aiohttp.TCPConnector(limit=MAX_CONNECTIONS)
        ...
        <p>
          <code>asyncio.to_thread()</code> moved the blocking calls to Python's default thread pool.
        </p>
        <p>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.95rem' }}>
            ...
            <tbody>
              <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                ...
                <td style={{ padding: '0.75rem 1rem' }}>Atomic directory claim by delivery UUID</td>
              </tr>
              <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
